...
std::unique_ptr<libwarp_state_struct> libwarp_state;
...
LIBWARP_ERROR_CODE libwarp_init() {
	if (!libwarp_state) {
		...
		libwarp_state = std::make_unique<libwarp_state_struct>();
		libwarp_state->did_init_libfloor = init_libfloor;
		
		// get compute context + device + create queue for it
		libwarp_state->ctx = floor::get_device_context();
		if(libwarp_state->ctx == nullptr) return LIBWARP_NO_CONTEXT;
		libwarp_state->dev = libwarp_state->ctx->get_device(device::TYPE::FASTEST);
		...
		if(libwarp_state->dev->max_total_local_size == 1024 &&
		   ...
		   libwarp_state->ctx->get_platform_type() != PLATFORM_TYPE::HOST) {
...
libwarp_build(const libwarp_camera_setup* const camera_setup) {
	...
	// build it
	auto program = std::make_shared<libwarp_state_struct::camera_setup_program>();
#if !defined(__WINDOWS__)
	const std::string kernel_file_name = "/opt/libwarp/include/libwarp/warp_kernels.hpp";
#else
	...
	program->program = libwarp_state->ctx->add_program_file(kernel_file_name,
															...
															" -DLIBWARP_SCREEN_HEIGHT=" + std::to_string(camera_setup->screen_height) +
															" -DLIBWARP_SCREEN_FOV=" + std::to_string(camera_setup->field_of_view) + "f" +
															" -DLIBWARP_NEAR_PLANE=" + std::to_string(camera_setup->near_plane) + "f" +
	...
	for(size_t i = 0; i < warp_kernel_count(); ++i) {
		program->kernels[i] = program->program->get_function(kernel_names[i]);
...
LIBWARP_ERROR_CODE libwarp_scatter_floor(const libwarp_camera_setup* const camera_setup,
										 ...
										 std::shared_ptr<device_image> color_texture,
										 std::shared_ptr<device_image> depth_texture,
										 std::shared_ptr<device_image> motion_texture,
										 std::shared_ptr<device_image> output_texture) REQUIRES(!libwarp_lock) {
	LIBWARP_INIT_AND_LOCK
...
LIBWARP_ERROR_CODE libwarp_gather_floor(const libwarp_camera_setup* const camera_setup,
										const float delta,
										std::shared_ptr<device_image> color_current_texture,
										std::shared_ptr<device_image> depth_current_texture,
										std::shared_ptr<device_image> color_prev_texture,
										std::shared_ptr<device_image> depth_prev_texture,
										std::shared_ptr<device_image> motion_forward_texture,
										std::shared_ptr<device_image> motion_backward_texture,
										std::shared_ptr<device_image> motion_depth_forward_texture,
										std::shared_ptr<device_image> motion_depth_backward_texture,
										std::shared_ptr<device_image> output_texture) REQUIRES(!libwarp_lock) {
	LIBWARP_INIT_AND_LOCK
