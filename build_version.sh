...
NEW_SOURCE_COUNT=$(find . -type f \( -name '*.h' -o -name '*.hpp' -o -name '*.cpp' -o -name '*.mm' -o -name '*.c' -o -name '*.m' \) -newer src/build_version | grep -v "build_version.hpp" | wc -l | tr -cd [:digit:])

BUILD_VERSION=$(cat src/build_version)
...
if [ ! -f src/build_version.hpp -o ${NEW_SOURCE_COUNT} -gt 0 ]; then
        echo "#define LIBWARP_BUILD_VERSION ${BUILD_VERSION}" > src/build_version.hpp
fi;
