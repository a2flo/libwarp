...
# only update build version if LIBWARP_DEV environment variable is set
if [ -n "${LIBWARP_DEV}" ]; then
...
# version of the target (preprocess the version header, grep the version defines, transform them to exports and eval)
eval $(${CXX} -E -dM include/libwarp/libwarp.h 2>&1 | grep -E "define (LIBWARP_(MAJOR|MINOR|REVISION|DEV_STAGE)_VERSION|LIBWARP_DEV_STAGE_VERSION_STR) " | sed -E "s/.*define (.*) [\"]*([^ \"]*)[\"]*/export \1=\2/g")
...
DEBUG_FLAGS="-O0 -DFLOOR_DEBUG=1 -DLIBWARP_DEBUG=1 -DDEBUG -fno-limit-debug-info"
if [ $BUILD_OS != "mingw" ]; then
